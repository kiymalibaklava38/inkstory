...
export default function PremiumPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const { data } = await supabase.from('profiles').select('is_premium, premium_expires_at').eq('id', user.id).single()
    ...
    <div className="min-h-screen bg-[var(--bg)] pb-20">
          ...
          <div className="rounded-2xl border-2 border-[var(--accent)] bg-[var(--card)] overflow-hidden relative">
            ...
            <div className="p-6">
              {userPlan === null ? (
                ...
                <button
                  className="w-full py-3 rounded-xl text-sm font-bold text-white transition-all hover:scale-[1.02] mb-6"
                  style={{ background: 'linear-gradient(135deg,#d4840f,#e8a030)' }}
      ...
      <div id="waitlist-section" className="max-w-lg mx-auto px-4 text-center">
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-8">
          ...
          {sent ? (
            <div className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm font-medium">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-xl border border-[var(--border)] bg-[var(--bg)] text-[var(--fg)] placeholder-[var(--fg-muted)] focus:outline-none focus:border-[var(--accent)] text-sm transition-all"
              />
              <button
                onClick={notify}
                disabled={!email.trim() || sending}
                className="flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-bold text-white transition-all hover:scale-105 disabled:opacity-50"
                style={{ background: 'linear-gradient(135deg,#d4840f,#e8a030)' }}
              >
                {sending
