...
export default function PremiumPage() {
  ...
  useEffect(() => {
    ...
    const loadUserPlan = async () => {
      ...
      if (error) {
        console.error('[PremiumPage] Failed to load profile:', error.message)
        setUserPlan('free')
        setCheckingPlan(false)
        return
      }
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      console.error('[PremiumPage] Checkout error:', error)
      ...
      <div className="max-w-4xl mx-auto px-4 mb-10">
        <div className="grid md:grid-cols-2 gap-5">
          <div className={`rounded-2xl border bg-[var(--card)] overflow-hidden transition-all ${userPlan === 'free' ? 'border-emerald-500/40' : 'border-[var(--border)]'}`}>
            ...
            <div className="p-6">
              {userPlan === 'free' ? (
                ...
                <Link
                  href="/register"
                  className="block w-full text-center py-3 rounded-xl text-sm font-semibold border border-[var(--border)] text-[var(--fg-muted)] hover:text-[var(--fg)] mb-6 transition-all"
                >
          ...
          <div className="rounded-2xl border-2 border-[var(--accent)] bg-[var(--card)] overflow-hidden relative">
            {userPlan === 'premium' ? (
              <div className="absolute top-0 left-0 right-0 text-center py-1.5 text-[11px] font-bold text-white" style={{ background: 'linear-gradient(135deg,#2d9f6a,#3dbd82)' }}>
                ✓ {lang === 'tr' ? 'Mevcut Planın' : 'Your Current Plan'}
              </div>
            ) : (
              <div className="absolute top-0 left-0 right-0 text-center py-1.5 text-[11px] font-bold text-white" style={{ background: 'linear-gradient(135deg,#d4840f,#e8a030)' }}>
            ...
            <div className="p-6">
              {checkingPlan || userPlan === null ? (
                <div className="w-full py-3 rounded-xl mb-6 flex items-center justify-center" style={{ background: 'linear-gradient(135deg,#d4840f,#e8a030)' }}>
                  <Loader2 style={{ width: 14, height: 14 }} className="animate-spin text-white" />
                </div>
              ) : userPlan === 'premium' ? (
                <div className="flex items-center justify-center gap-2 w-full py-3 rounded-xl text-sm font-bold text-white mb-6" style={{ background: 'linear-gradient(135deg,#2d9f6a,#3dbd82)' }}>
          ...
          {[
            {
              ...
              a: lang === 'tr'
                ? 'Evet, aboneliğinizi istediğiniz zaman iptal edebilirsiniz. İptal sonraki fatura döneminde geçerli olur.'
                : 'Yes, you can cancel your subscription at any time. Cancellation takes effect at the next billing period.',
            },
            {
              q: lang === 'tr' ? 'İade politikası nedir?' : 'What is the refund policy?',
              a: lang === 'tr'
                ? 'İlk 14 gün içinde tam iade garantisi. Aylık planlarda sonraki dönem için otomatik iptal, yıllık planlarda kalan süre oranında iade.'
                : 'Full refund within the first 14 days. Monthly plans get automatic cancellation, yearly plans get prorated refunds.',
            },
            {
              q: lang === 'tr' ? 'Hangi ödeme yöntemleri kabul ediliyor?' : 'What payment methods are accepted?',
              a: lang === 'tr'
                ? 'Visa, Mastercard, American Express, PayPal ve diğer yerel ödeme yöntemleri Paddle üzerinden kabul edilmektedir.'
                : 'Visa, Mastercard, American Express, PayPal and other local payment methods are accepted via Paddle.',
            },
            {
              q: lang === 'tr' ? 'Premium özelliklere hemen erişebilir miyim?' : 'Do I get instant access to Premium features?',
              a: lang === 'tr'
                ? 'Evet. Ödeme Paddle tarafından başarıyla işlendiğinde üyeliğiniz aktif olur.'
                : 'Yes. Your membership becomes active after Paddle successfully processes the payment.',
            },
          ].map((item, i) => (
